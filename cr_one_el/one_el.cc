...
int main(int argc, char* argv[])
{
  ...
  one_el ana(inputFileList, outFileName, data);
...
void one_el::EventLoop(const char *data,const char *inputFileList)
{ 
  if (fChain == 0) return;
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++)
	...
	{goodphoton = goodphotons[0];}
      ...
      if(Electrons->size()!=1) continue;  // 1 reco electrons events will pass this cut
      if(Electrons_passIso!=1) continue;
      ...
      if(mindr<0.3)
	{
      ...
      // // ----------------------  Gen level -------------------------                      ( gen level selection is not needed ) 
      ...
      // if(gen_mu.size()!=0) continue; // rejecting hadronic decays
      // if(gen_el.size()==0) continue; // only events with gen level electrons are selected
      // if(Electrons->size()==0)
      // 	{
      ...
      // // checking if the photon is real or fake   ( no need for this ) 
      ...
      // // electron matching with jets and lost in a jet ( Reco electron collection already has isolation )
	...
	{ survived_events+=wt;
	  ...
	  one_electron->Fill("NJets_{=0}^{2-4}",0);
	  one_electron->Fill("NJets_{#geq 1}^{2-4}",0);
	  one_electron->Fill("NJets_{=0}^{5-6}",0);
	  one_electron->Fill("NJets_{#geq 1}^{5-6}",0);
	  one_electron->Fill("NJets_{=0}^{#geq 7}",0);
	  one_electron->Fill("NJets_{#geq 1}^{#geq 7}",0);

	  int njets = goodjets.size();

	  one_el_2->Fill("NJets_{0}^{=2} & 100<MET<150",0);
	  one_el_2->Fill("NJets_{0}^{=2} & MET#geq 150",0);
	  one_el_2->Fill("NJets_{0}^{=3} & 100<MET<150",0);
	  one_el_2->Fill("NJets_{0}^{=3} & MET#geq 150",0);
	  one_el_2->Fill("NJets_{0}^{=4} & 100<MET<150",0);
	  one_el_2->Fill("NJets_{0}^{=4} & MET#geq 150",0);
	  one_el_2->Fill("NJets_{0}^{5-6} & 100<MET<150",0);
	  one_el_2->Fill("NJets_{0}^{5-6} & MET#geq 150",0);
	  one_el_2->Fill("NJets_{0}^{#geq7} & 100<MET<150",0);
	  one_el_2->Fill("NJets_{0}^{#geq7} & MET#geq 150",0);
	  one_el_2->Fill("NJets_{#geq 1}^{2-4} & 100<MET<150",0);
	  one_el_2->Fill("NJets_{#geq 1}^{2-4} & MET#geq 150",0);
	  one_el_2->Fill("NJets_{#geq 1}^{5-6} & 100<MET<150",0);
	  one_el_2->Fill("NJets_{#geq 1}^{5-6} & MET#geq 150",0);
	  one_el_2->Fill("NJets_{#geq 1}^{#geq 7} & 100<MET<150",0);
	  one_el_2->Fill("NJets_{#geq 1}^{#geq 7} & MET#geq 150",0);
	  	    
	  
	  if(BTags==0)
	    { if(goodjets.size()>=2 && goodjets.size()<=4)
		{		  
		  one_electron->Fill("NJets_{=0}^{2-4}",wt);
		  if(njets==2)
		    {
		      if(MET>=100 && MET<150)
			{ one_el_2->Fill("NJets_{0}^{=2} & 100<MET<150",wt);}
		      if(MET>=150)
			{ one_el_2->Fill("NJets_{0}^{=2} & MET#geq 150",wt);}
		    }
		  if(njets==3)
		    {
		      if(MET>=100 && MET<150)
			{ one_el_2->Fill("NJets_{0}^{=3} & 100<MET<150",wt);}
		      if(MET>=150)
			{ one_el_2->Fill("NJets_{0}^{=3} & MET#geq 150",wt);}
		    }
		  if(njets==4)
		    {
		      if(MET>=100 && MET<150)
			{ one_el_2->Fill("NJets_{0}^{=4} & 100<MET<150",wt);}
		      if(MET>=150)
			{ one_el_2->Fill("NJets_{0}^{=4} & MET#geq 150",wt);}
		    }
		}
	      if(goodjets.size()>=5 && goodjets.size()<=6)
		{
		  one_electron->Fill("NJets_{=0}^{5-6}",wt);
		  if(MET>=100 && MET<150)
		    { one_el_2->Fill("NJets_{0}^{5-6} & 100<MET<150",wt);}
		  if(MET>=150)
		    { one_el_2->Fill("NJets_{0}^{5-6} & MET#geq 150",wt);}
		}
	      if(goodjets.size()>=7)
		{
		  one_electron->Fill("NJets_{=0}^{#geq 7}",wt);
		  if(MET>=100 && MET<150)
		    { one_el_2->Fill("NJets_{0}^{#geq7} & 100<MET<150",wt);}
		  if(MET>=150)
		    { one_el_2->Fill("NJets_{0}^{#geq7} & MET#geq 150",wt);}
		}
	  ...
	  if(BTags>=1)
	    { if(goodjets.size()>=2 && goodjets.size()<=4)
		{
		  one_electron->Fill("NJets_{#geq 1}^{2-4}",wt);
		  if(MET>=100 && MET<150)
		    { one_el_2->Fill("NJets_{#geq 1}^{2-4} & 100<MET<150",wt);}
		  if(MET>=150)
		    { one_el_2->Fill("NJets_{#geq 1}^{2-4} & MET#geq 150",wt);}
		}
	      if(goodjets.size()>=5 && goodjets.size()<=6)
		{
		  one_electron->Fill("NJets_{#geq 1}^{5-6}",wt);
		  if(MET>=100 && MET<150)
		    { one_el_2->Fill("NJets_{#geq 1}^{5-6} & 100<MET<150",wt);}
		  if(MET>=150)
		    { one_el_2->Fill("NJets_{#geq 1}^{5-6} & MET#geq 150",wt);}
		}
	      if(goodjets.size()>=7)
		{
		  one_electron->Fill("NJets_{#geq 1}^{#geq 7}",wt);
		  if(MET>=100 && MET<150)
		    { one_el_2->Fill("NJets_{#geq 1}^{#geq 7} & 100<MET<150",wt);}
		  if(MET>=150)
		    { one_el_2->Fill("NJets_{#geq 1}^{#geq 7} & MET#geq 150",wt);}
		}
...
bool one_el::electron_match_photon(TLorentzVector photon)
